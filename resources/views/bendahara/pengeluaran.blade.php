<x-layouts.app title="Pengeluaran Kas">
    <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start gap-3 mb-4">
        <div>
            <h1 class="h4 fw-bold mb-1">Pengeluaran Kas</h1>
            <p class="text-muted small mb-0">Catat semua pengeluaran (Kredit)</p>
        </div>
        <button type="button" class="btn btn-danger fw-semibold" data-bs-toggle="modal" data-bs-target="#modalPengeluaran"><i class="bi bi-dash-circle me-1"></i>Catat Pengeluaran</button>
    </div>
                ...
                <tbody>
                    @forelse($transactions as $trx)
                        <tr>
                            ...
                            <td class="small text-end fw-bold text-danger">- Rp {{ number_format($trx->amount, 0, ',', '.') }}</td>
                    ...
                    @empty
                        <tr><td colspan="5" class="text-center text-muted small py-4">Belum ada data pengeluaran</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title fw-bold" id="modalPengeluaranLabel">Catat Pengeluaran Baru</h5>
                ...
                <form action="{{ route('bendahara.pengeluaran.store') }}" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" id="description" required rows="3" class="form-control" placeholder="Contoh: Beli Spidol, Sapu, Fotocopy, dll."></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-outline-secondary" data-bs-dismiss="modal">Batal</button>
                        <button type="submit" class="btn btn-danger fw-semibold">Simpan Pengeluaran</button>
                    </div>
                </form>
            </div>
        </div>
